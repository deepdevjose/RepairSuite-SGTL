...
export function DashboardHeader({ title }: DashboardHeaderProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 h-14 bg-gradient-to-b from-slate-900/95 to-slate-900/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 gap-4">
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-white/10 rounded-lg shadow-xl backdrop-blur-xl z-50 overflow-hidden">
              {isSearching ? (
                ...
                searchResults.map((result, index) => (
                  <button
                    key={index}
                    className="w-full px-4 py-3 flex items-start gap-3 hover:bg-white/10 active:bg-white/15 transition-all duration-200 text-left border-b border-white/5 last:border-0 group cursor-pointer"
                    onClick={() => {
                      setSearchQuery("")
                      setShowSearchResults(false)
          ...
          <div className="mt-6 space-y-3">
            {loading ? (
              ...
              notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`p-4 rounded-lg border transition-all duration-200 hover:bg-white/5 cursor-pointer ${!notification.leida
